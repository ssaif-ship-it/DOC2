...
**Using Cashfree's SDK, or Web Checkout.** This is the default integration path and is sufficient for most merchants. No additional code is required to present the UPI application picker; the operating system handles this natively once the link is triggered. This applies to the Android, iOS, React Native, and Flutter SDKs, as well as to Cashfree's hosted Web Checkout accessed from a mobile browser.
...
**Building a fully custom checkout.** Merchants who do not use Cashfree's SDK or hosted Web Checkout, and instead load Cashfree's checkout page inside their own application's WebView, are responsible for intercepting the UPI deep link directly. The implementation differs by platform:
...
<!-- Claude, flagging for Saif: the Android/iOS Custom Checkout details were re-verified on 2026-08-28 directly against the live cashfree.com/docs/payments/online/web/custom-checkout-android and -ios pages, all technical points match. Still unconfirmed: whether "Seamless" is a distinct integration mode with its own deep-link enumeration, separate from the SDK/Web Checkout vs Custom Checkout split used here, that is what Ayushi's original comment asked about specifically, and needs a product/eng answer rather than another doc lookup. -->
