UPI Intent is the recommended payment flow for customers checking out on a mobile device, whether through your mobile website or your own application. It requires only two actions from the customer: selecting a UPI application, and entering their PIN.
...
The customer selects Pay on the checkout page. The operating system opens a list of the UPI applications installed on the device. On selecting one, it opens with the payment amount, payee, and account details already populated. The customer enters their UPI PIN, and control returns to the checkout page.

### Why this flow is recommended

UPI Intent avoids the two points of friction present in other UPI flows: manual entry of a VPA, which is prone to input errors, and QR code scanning, which is not possible when the customer is checking out on the same device. Because UPI Intent requires no manual data entry and no application switching outside the guided flow, it consistently completes at a higher rate than flows that do.

### How it works

UPI Intent is triggered by a deep link beginning with `upi://pay?...`. When the customer selects Pay, the checkout constructs this link and passes it to the operating system, which resolves it against every UPI application installed on the device and presents them as options. Cashfree and the selected UPI application manage the remainder of the transaction; the integration is responsible only for triggering the link and confirming the result.

### Integration options

**Using Cashfree's SDK, or Web Checkout in a browser.** This is the default integration path and is sufficient for most merchants. No additional code is required to present the UPI application picker; the operating system handles this natively once the link is triggered. This applies to the Android, iOS, React Native, and Flutter SDKs, as well as to Cashfree's hosted Web Checkout when it is opened in a mobile browser or through a full page redirect, rather than embedded inside the merchant's own application.

One platform difference is worth noting. On Android, the `upi://pay` link is resolved consistently, since every major UPI application registers for this scheme. On iOS, the link behaves the same way in Safari and within a native application, but is less reliable inside certain in-app browsers, such as the webview used by social apps for links opened from a feed, and in some other embedded browser environments. In these cases, iOS may not raise an error, and the link can fail without any visible response to the customer. Merchants with a meaningful share of iOS traffic in these contexts should not rely on UPI Intent as the only flow. UPI Collect ([section 2.2](#doc-2-2)) remains available on iOS, and Dynamic QR ([section 2.3](#doc-2-3)) is an additional alternative.

**Loading Web Checkout inside the merchant's own WebView.** Some merchants embed Cashfree's Web Checkout page inside their own Android, iOS, React Native, Flutter, or Cordova application using a WebView, rather than opening it in a system browser. A WebView does not hand off to other applications the way a system browser does, so without additional code the UPI option can appear on the checkout page and do nothing when selected. Cashfree's Web Checkout still renders the page in this case, only the deep link interception is the merchant's responsibility. The implementation differs by platform:
...
**Integrating directly against the API.** Merchants who do not use Cashfree's SDK or Web Checkout at all, and instead build their own checkout interface from the ground up, retrieve the deep link from the API rather than from a rendered checkout page. The deep link is not included in the Create Order response. It is returned by the Order Pay call, in the payload for the requested UPI payment method. Setting `payment_method.upi.channel` to `"link"` returns the `upi://pay?...` string directly. Refer to the API Reference for the current field name and response schema, as this is versioned and subject to change. If this custom interface is itself embedded in a WebView, the interception steps described above still apply.
